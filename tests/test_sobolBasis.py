...
np.all(
    T_var[0]
    == np.sum(
        [S_var[i] for i in range(len(S_var)) if str(1) in sob.names_ind[i]], axis=0
    )
)
np.all(
    T_var[1]
    == np.sum(
        [S_var[i] for i in range(len(S_var)) if str(2) in sob.names_ind[i]], axis=0
    )
)
np.all(
    T_var[2]
    == np.sum(
        [S_var[i] for i in range(len(S_var)) if str(3) in sob.names_ind[i]], axis=0
    )
)
np.all(
    T_var[3]
    == np.sum(
        [S_var[i] for i in range(len(S_var)) if str(4) in sob.names_ind[i]], axis=0
    )
)
### etc
np.all(
    T_var[8]
    == np.sum(
        [S_var[i] for i in range(len(S_var)) if str(9) in sob.names_ind[i]], axis=0
    )
)
