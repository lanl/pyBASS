...
def getCombs(mod, uniq_models, nmodels, max_basis, max_int_tot, func_var=None):
    ...
    for i in uniq_models:
        for j in range(mod.samples.nbasis[uniq_models[i]]):
            temp = sorted(mod.samples.vs[i,j,:mod.samples.n_int[i,j]].tolist())
            if temp not in n_un:
                n_un.append(temp)
    ...
    for el in int_lower:
        if el not in n_un:
                n_un.append(el) # add lower order interactions if they are not there already
    ...
    int_begin_ind.append(len(n_un))  # need the top level to help with indexing later
    ...
    for i in ints_used:
        if int_begin_ind[i-1] is not None:
            mat = np.array([n_un[j] for j in range(int_begin_ind[i-1], int_begin_ind[i])])
            mat = mat[np.lexsort(mat.T), :]
            combs.append(mat.T)
            names_ind.append(['x'.join(map(str, x)) for x in mat])
            disp_combs = np.array([[labs[y] for y in x] for x in mat])
            disp_combs = np.apply_along_axis(lambda x: sorted(x, key=lambda y: (isinstance(y, int), y)), axis=1, arr=disp_combs)
            if i==1:
                disp_names.append(disp_combs)
            else:
                disp_names.append(['x'.join(map(str, x)) for x in disp_combs])

    num_ind = [x.shape[1] for x in combs]  # num_ind[i] is number of interactions of order i
    cs_num_ind = np.cumsum(num_ind)  # used for indexing

    return {'combs': combs, 'names_ind': names_ind, 'num_ind': num_ind, 'cs_num_ind': cs_num_ind, 'disp_combs': disp_combs, 'disp_names': disp_names}
            ...
            for m1 in bases_v_idx:
                ...
                for m2 in range(nb):
                    if m2 in bases_v_idx:
                        ...
                        vind2 = np.where(mod.samples.vs[mod_idx, m2, :nint2] == v)[0]
                        sign2 = mod.samples.signs[mod_idx, m2, vind2]
                        knot2 = mod.samples.knots[mod_idx, m2, vind2]
                        C2arr[m1, m2, v] = C2arr[m2, m1, v] = C2(v, sign1, sign2, knot1, knot2, xx)
                    else:
...
def get_Cmats_sub(mod, mod_idx, mcmc_use, xx, subsets):
    subvec = np.array([next(i for i, s in enumerate(subsets) if j in s) for j in range(mod.data.p)])
    ...
    for u in range(len(subsets)):
        ...
        if len(bases_u_idx) > 0:
            for m1 in bases_u_idx:
                nint1 = mod.samples.n_int[mod_idx, m1]
                vind1 = np.where(np.isin(mod.samples.vs[mod_idx, m1, :nint1], subsets[u]))[0]
                ...
                for m2 in range(nb):
                    if m2 in bases_u_idx:
                        nint2 = mod.samples.n_int[mod_idx, m2]
                        vind2 = np.where(np.isin(mod.samples.vs[mod_idx, m2, :nint2], subsets[u]))[0]
                        vars2 = mod.samples.vs[mod_idx, m2, vind2]
                        sign2 = mod.samples.signs[mod_idx, m2, vind2]
                        knot2 = mod.samples.knots[mod_idx, m2, vind2]
                        C2arr[m1, m2, u] = C2arr[m2, m1, u] = C2sub(vars1, vars2, sign1, sign2, knot1, knot2, xx)
                    else:
...
def get_sob(mcmc_use, mod, xx, ncores):
    ...
    if len(combs) > 1:
        for l in range(1, len(combs)):
            for i in range(combs[l].shape[1]):
                vces_normed[:, k] = vce_combine(combs[l][:, i], combs, cs_num_ind, vces)
                k += 1
...
def get_sob_sub(mcmc_use, mod, xx, subsets):
    ...
    """
    models = mod.model_lookup[mcmc_use]  # only do the heavy lifting once for each model
    uniq_models = np.unique(models)
    nmodels = len(uniq_models)
    maxInt_tot = np.sum(mod.prior.maxInt)
    maxBasis = np.max(mod.samples.nbasis)
    ...
    for i in range(len(combs)):
        for j in range(combs[i].shape[1]):
            subcombs.append([k for k, s in enumerate(subsets) if any(item in s for item in combs[i][:, j])])
    ...
    subcombs = [np.column_stack([subcombs[j] for j in range(len(subcombs)) if len(subcombs[j]) == i]) for i in range(1, max(subcombs_size)+1)]
    ...
    for mod_idx in uniq_models:
        ind = np.where(models == mod_idx)[0]
        cmats = get_Cmats_sub(mod=mod, mod_idx=mod_idx, mcmc_use=mcmc_use, xx=xx, subsets=subsets)
        k = 0
    ...
    if len(subcombs) > 1:
        for l in range(1, len(subcombs)):
            for i in range(subcombs[l].shape[1]):
                vces_normed[:, k] = vce_combine(subcombs[l][:, i], subcombs, subcs_num_ind, vces)
                k += 1

    out = pd.DataFrame((vces_normed.T / tot_var).T)
    out.columns = ['x'.join(map(str, subcombs[l][:, i])) for l in range(len(subcombs)) for i in range(subcombs[l].shape[1])]
...
def sobol_MC(mod, mcmc_use=None, xx=None, ncores=1):
    ...
    return {'S': sob['S'].iloc[:, ord], 'T': get_tot_MC(sob['S'].iloc[:, ord]), 'tot_var': sob['tot_var']}
